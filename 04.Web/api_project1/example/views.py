...
def booksAPI(request):
    if request.method == "GET": # get 요청(도서 전체 정보 제공)
...
def bookAPI(request, bid):
    book = get_object_or_404(Book, bid = bid) # bid = pk인 데이터를 Book에서 가져오고, 없으면 404 에러
...
class BooksAPIMixins(mixins.ListModelMixin, mixins.CreateModelMixin, generics.GenericAPIView):
    queryset = Book.objects.all() # 조회 할 데이터
    serializer_class = BookSerializer # api에서 사용할 시리어라이저 설정 

    def get(self, request, *args, **kwargs): # get 메소드 처리 함수(전체목록 조회)
...
class BookAPIMixins(mixins.RetrieveModelMixin, mixins.UpdateModelMixin, mixins.DestroyModelMixin, generics.GenericAPIView):
    ...
    def put(self, request, *args, **kwargs): # put 메소드 처리 함수(1권 수정)
        return self.update(request, *args, **kwargs) # mixins.UpdateModelMixin 과 연결
